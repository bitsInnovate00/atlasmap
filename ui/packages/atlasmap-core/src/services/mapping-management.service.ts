...
console.log("fieldSelected..")
    ...
    if (mapping !== null) {
      console.log("mapping !== null");
      if (mapping.hasMappedField(field.isSource())) {
        console.log("mapping.hasMappedField(field.isSource())");
  ...
  enableMappingPreview(): void {
    ...
    this.mappingPreviewInputSubscription = this.mappingPreviewInput$.subscribe(
      (inputFieldMapping) => {
        ...
        console.log('Process Mapping Preview Request');
        this.cfg.logger!.debug(
  ...
  async notifyMappingUpdated(): Promise<boolean> {
    console.log("notifyMappingUpdated");
    return new Promise<boolean>(async (resolve) => {
      if (this.cfg.mappings) {
